 ...
 *
 */
export default function getAgOptions({grid, columnLibrary, state, data, filterModel}) {
  ...
  const options = {
    ...
    onGridReady(params) {
      ...
      // default sort order
      const validColumns = [
        ...Object.keys(columnLibrary),
        ...mainState.columns.filter(v => /^c-\d{1,2}$/.test(v.id)).map(v => v.id)
      ];
      const sortId = validColumns.includes(mainState.sort.colId) ? mainState.sort.colId : validColumns[0];
      params.api.setSortModel([
        {
          colId: sortId,
          sort: mainState.sort.sort
        }
      ]);
    ...
    onSortChanged(params) {
      ...
      window.bo.inst.state.set({gadgetId: state.id, handler: oldGadgetState => {
        oldGadgetState.sort = sortModel;
