...
import formatterSnapshot         from './formatter-snapshot.js';
...
import getJobsSnapshot           from './get-jobs-snapshot.js';
...
let snapshot;
{
  const key = (str, ob) => `${scrapeDir}/cryptocompare-snapshot-grouped/data.json`;
  snapshot = {
    event: 'data',
    name: 'snapshot',
    interval: 1000 * 30,
    watchDirs: [key`${{}}`, 'all'],
    getJobs: getJobsSnapshot,
